...
const { deleteStartPoint, deleteEndPoint, autocompleteInputs, startPoint, endPoint, waypoints, loadingAPI, errorAPI, successAPI } = useAutocomplete()
...
<template>
    <template v-if="loadingAPI && !errorAPI && !successAPI">
        <div class="grid justify-center grid-cols-4 col-span-12 gap-4 px-4 pt-4 min-h-[620px]">
            <div class="flex flex-col items-center justify-center col-span-4 gap-4">
                <LoadingIcon icon="tail-spin" class="w-16" />
            </div>
        </div>
    </template>
    <template v-if="!loadingAPI && errorAPI && !successAPI" >
        <div class="grid justify-center grid-cols-4 col-span-12 gap-4 px-4 pt-4 min-h-[620px]">
            <div class="flex flex-col items-center justify-center col-span-4 gap-4">
                <h2 class="flex flex-row items-center justify-center gap-1 text-2xl font-bold text-red-600">
                    <Lucide icon="TriangleAlert" class="w-8 h-8" />
                    <span>
                        Server Error
                    </span>
                    <Lucide icon="TriangleAlert" class="w-8 h-8" />
                ...
                <button class="flex flex-row items-center justify-center gap-2 px-4 py-2 text-white bg-blue-500 rounded-lg hover:bg-blue-600 focus:outline-none"
                    @click="reload">
    ...
    <div :class="{ 'hidden': loadingAPI || errorAPI || !successAPI }" class="grid grid-cols-4 col-span-12">
